...
print(delay_by_origin %>% 
      select(city, state, n_voli, ritardo_medio, perc_ritardo) %>%
      head(70))
...
recovery_by_route <- flight_recovery %>%
  ...
  summarise(
    ...
    recupero_mediano = median(time_recovery),
    perc_recupera = sum(time_recovery > 0) / n() * 100,
    .groups = "drop"
  ) %>%
  filter(n_voli >= 100) %>%
  left_join(airports %>% select(airport_id, city, state), 
...
print(recovery_by_route %>%
      mutate(rotta = paste(origin_city, "→", dest_city)) %>%
      select(rotta, n_voli, recupero_medio, perc_recupera) %>%
...
```{r}
# Esegui K-means con numero ottimale (es. 4 cluster)
set.seed(42)
k <- 3
...
Vado a scrivermii la funzione Percolate che abbiamo visto in classe, e in più mi vao a scrivere anche la funzione che mi va a calcolare ogni volta tolto un nodo la betwweenness
```{r}
...
Eseguo gli attacchi, e stampo i nodi che le varie matriche vanno a togliere per prima così da vedere le differenze 
```{r}
# ESEGUI ATTACCHI
...
**Tutte le strategie mostrano curve quasi sovrapposte** nelle prime rimozioni. Questo fenomeno è **assolutamente normale** e rivela tre caratteristiche fondamentali della rete aerea USA:
...
Il fatto che **Power Centrality rimuova nodi diversi ma ottenga lo stesso risultato iniziale** è la prova più convincente dell'**alta ridondanza** del sistema aeroportuale americano.
